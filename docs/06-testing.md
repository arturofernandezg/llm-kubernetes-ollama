...
|---|---|---|---|
| `test_endpoints.py` | 137 | Integración | Health probes (`TestReadyzQueueMode` ×3: readyz Redis-gated), /extract end-to-end, retry con backoff, /webhook/alert (encola, fail-closed 503 si Redis None), /metrics, formatter, feedback loop, `/webhook/action` (FakeRedis, approve/reject/HMAC + **fail-closed/fail-open gated por dry_run** ×2, v2 P0·2), fallback isolation, `TestWebhookQueuePath` (×4) + `TestHandleStreamEntry` + `TestPeriodicReclaim`, `TestDiagnosisTimeout` (×4), `TestEscalationStoreMetric` (×2, PR-06), `TestRagReconnect` (×2, PR-05), slash `/webhook/command` fail-closed (×1, v2 P0·2). **C-02** (×3, `TestApproveStructuredParity`: el approve siembra cooldown). **C-07** (×2 wiring: all-denied→notificación sin botones; parcial→solo el ejecutable es aprobable). **C-08**: `TestCommandVariantEscalation` (×1, E2E doble variante + persistencia `command_variants`) + `TestCommandVariantCallback` (×3: approve_model/approve_engine ejecutan el comando correcto; token de una acción NO autoriza la otra → 401) |
...
| `test_diagnosis.py` | 31 | Unitario | build_alert_text, format_context_docs, generate_diagnosis, _clamp (LLM mockeado); `test_generation_uses_configured_temperature` (temp=0 viaja en `options.temperature`); `TestFormatClusterFacts` (×5, cluster facts fail-soft, incl. `restart_count=0` = hecho real) + 2 de integración (snapshot entra en el prompt / sin snapshot prompt limpio). **R2·3** (×5): labeling `cured`/`rolled_back`/`rollback_failed` en `format_context_docs`, runbook sin outcome intacto, guard del contrato `DIAGNOSIS_PROMPT` (label y regla `FAILED FIX` evolucionan juntos). **F-17** (×2): bloques RECENT LOGS/EVENTS en `format_cluster_facts` |
| `test_remediation.py` | 245 | Unitario | classify_command (SAFE/MUTATING/BLOCKED/UNKNOWN), validate_commands, decide_action (9 reglas incl. 4.5/4.6 + 7.5 `rag_degraded`→escalate, PR-04), execute_commands (dry-run + real mode mock), process_remediation, _get_safe_commands, zero_current_memory. **F3**: `TestParseCpuToMillicores`, `TestDecideActionCpu` (escalate-first), `TestDecideActionCpuAuto` (flag on/off), `TestLimitResource`, `TestCapturePrePatchValue`/`TestRevertPatch` cpu (jsonpath/`--limits=cpu` field-aware), `TestProcessRemediationCpuAuto` (enganche E2E determinista), `TestStructuredAutoRemediation` (re-sourcing: risk=high+conf≥0.8+proposed_action → AUTO por vía estructurada; guardrail solo-subir + namespace ajeno). **v2 Eje A + hardening**: `TestSealProposedAction` (sella identidad/valor/match_labels; anula PA si workload sin confirmar o kind≠Deployment F-02; síntesis `new_value=2×current`), `TestDeriveConfidence`/`TestGroundConfidence` (confidence grounded + `model_confidence`), regla 4.7 `target_unresolved`, selector real en capture/revert. **F-01**: `TestWorkloadCooldown` (×6: adquiere→AUTO con key/nx/ex correctos, bloqueado→ESCALATE sin ejecución, Redis error→ESCALATE fail-closed, `redis_client=None`→pasa, rama no estructurada no consulta, helper key+TTL). **C-01** (×4 + 2 adaptados: `check_pod_health` con `lastState.terminated.reason`, solo `_FAILURE_REASONS` marca unhealthy, formato jsonpath de 3 campos). **C-07** (~15): `TestAuthCanIArgs` (mapeo SAFE→verb/resource, `top pod`→`pods.metrics.k8s.io`) + `TestCheckCommandExecutable` (False solo ante "no" explícito, fail-open en error/timeout) + `TestPartitionByPermission`. **C-08** (×4): `TestStructuredCommandVariants` (2 variantes con overshoot, 1 si model==2×current, 1 si current no doblable, `[]` si no estructurado) |
...
| `test_enrichment.py` | 35 | Unitario | Grounding v2 (Eje A): `_kubectl_json` (fail-soft: rc≠0/timeout/JSON inválido/excepción → None, `proc.kill()` en timeout), parseo del pod (limits/phase/restart_count/last_state_reason), selección determinista de container, `_controller_owner`, `_resolve_workload` (cadena pod→RS→Deployment, gate de existencia, STS/DS directo). **F-17** (~10): `TestGatherLogs` (`--previous` si el contenedor murió/reinició, fallback a current, caps líneas/chars) + `TestGatherEvents` (field-selector, newest-last, límite) + wiring en `gather_incident_context` (cada gather fail-soft por separado) |
...
| `test_incident_index.py` | 15 | Unitario | **R5 (bucle observacional)**: módulo `incident_index.py` (roundtrip record/pop, pop consume una sola vez, fail-open ×3 con Redis None/caído), `_correlate_resolution` (hit no-auto re-upserta a `resolved_observed` / hit `awaits_verdict` solo métrica / miss / campos incompletos / fail-open ante ingest caído / `started_at` no numérico), `_alert_fingerprint` (fuente única), wiring (un escalate queda indexado con `awaits_verdict=False`) |
| `test_eval_feedback.py` | 16 | Unitario | **R4 (harness feedback-loop)**: fixture `incidents_seed.json` (validación de shape + disclosure sintético), extracción de outcome, retrieval de clases mockeado (Capa A), fallback runbooks two-stage — offline, sin cluster |
| **Total** | **695** | | **pytest colecta 700, verde Jay-confirmado 2026-07-15** (`pytest agent/tests/ -q` → **700 passed**; parametrize expande ~5 sobre las 695 funciones). Incluye los ~4 de S7 (audit ×2, C-08 render/sanitize ×2 — ver bullet S7 abajo). Los builds de S7 (`7e40837`, `2ac3c5d`) pasaron el gate de Cloud Build. |
...
- F4 R2/R2·3/R3 + gate E4 + F-03 (2026-07-03/04): bloque RAG (re-upsert del veredicto, retrieval outcome-aware, higiene de query) + `TestChromaOffloading` (F-03, spy sobre `rag.asyncio.to_thread`). **Total reconciliado a 620** (Jay-confirmado verde 2026-07-04, `8a40fdc`; los docs venían diciendo ~573/613 — recuento por fichero rehecho).
- Cierre gap R2-humano (2026-07-06, `ca159be`): +1 `test_approve_feeds_verdict_loop_with_doc_id` en `test_endpoints.py::TestApproveStructuredParity` — el approve pasa `doc_id`+`remediation` a `_schedule_rollback_evaluation` y el ingest final reusa el doc con `auto_pending`. **Total 621** (Jay-confirmado verde 2026-07-06, `0914611`).
- R4 harness (2026-07-08): nuevo `test_eval_feedback.py` (×16, offline: fixture + extracción de outcome + retrieval mockeado).
- Quick-wins C-01/C-02 (2026-07-09): C-01 +4 (y 2 adaptados al jsonpath de 3 campos `Running|0|;`) en `test_remediation.py`; C-02 +3 en `test_endpoints.py::TestApproveStructuredParity`. **Gotcha**: `helpers.FakeRedis` no cubría `set(nx=,ex=)` — código nuevo con SETNX contra FakeRedis falla en silencio (fail-soft) si no se añade el método espejo; añadido.
- R5 bucle observacional (2026-07-10, `1dbfdf3`): nuevo `test_incident_index.py` (×15).
- F-17 + C-07 (2026-07-10, `588e3a9`): F-17 ~10 en `test_enrichment.py` (`TestGatherLogs`/`TestGatherEvents` + wiring) + 2 en `test_diagnosis.py` (bloques RECENT LOGS/EVENTS); C-07 ~15 en `test_remediation.py` (`TestAuthCanIArgs`/`TestCheckCommandExecutable`/`TestPartitionByPermission`) + 2 wiring en `test_endpoints.py`.
- C-08 doble botón (2026-07-10, `588e3a9`): +4 `TestStructuredCommandVariants` en `test_remediation.py` + 4 en `test_endpoints.py` (`TestCommandVariantEscalation` ×1 + `TestCommandVariantCallback` ×3, incl. integridad HMAC por acción). **Total 691 funciones / 17 ficheros; pytest colecta 696** (Jay-confirmado verde 2026-07-11: 692 + 4 arregladas en la sesión del pytest full).
- S7 (2026-07-13, `7e40837`+`2ac3c5d`): audit trail +2 en `test_endpoints.py` (log estructurado en approve/reject) + fix C-08 en `test_mattermost.py`: `test_c08_variant_button_ids_are_alphanumeric` (ejercita el **render real** de `send_escalation_with_buttons` con `approve_variants`) + `test_sanitize_action_id_*`. **Lección del bug underscore→404**: los tests C-08 previos mockeaban `send_escalation_with_buttons` y solo verificaban `context.action` — nadie ejercitaba el payload del botón (`id`); el mock se saltaba justo donde vivía el bug. **Total 695 funciones / 17 ficheros; pytest colecta 700, verde Jay-confirmado 2026-07-15 (700 passed).**
