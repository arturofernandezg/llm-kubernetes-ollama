...
- Sistema **completo**; en cluster corre `aiops-agent:da7aafb`; la imagen **`cb2d1db` está buildeada pero NO desplegada** (bloqueada en recuperación de secrets — un `secrets-setup.sh` con placeholders pisó el webhook real; runbook de recuperación en la bitácora 2026-07-02). Fases 0-3 + Mini-Fase 4 + RAG + F2 (cola) + F3 (CPU + re-sourcing) + **v2 completa en código** (Ejes A+B). **565 tests en 15 ficheros.**
...
- **En curso / siguiente**: (1) Jay: pytest + commit + push (activa CI) + recuperación secrets + deploy `set image` + **chaos OOM del arco completo** (validar: seal desde snapshot, `aiops_enrichment_total{gathered}`, sin `NotFound`, confidence grounded en MM); (2) mejoras RAG analizadas 2026-07-03 (R1 filtro metadata → R3 higiene query → R2 bucle aprendizaje real → R4 métrica gain — detalle en `docs/07`); (3) H1 restante de la review; (4) F1 matriz E1–E6 (`docs/14`); Gate 8 screenshots.
