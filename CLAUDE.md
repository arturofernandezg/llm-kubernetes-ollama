...
- Sistema **completo**; en cluster corre y está **validado `aiops-agent:8a40fdc`** (desplegado 2026-07-04, 1ª vez que la v2 completa corre en cluster). Fases 0-3 + Mini-Fase 4 + RAG + F2 (cola) + F3 (CPU + re-sourcing) + **v2 completa** (Ejes A+B) + **F4 R1/R2/R2·3/R3** (retrieval outcome-aware, p@1 73%→100%) + **F-03** (chroma no bloqueante). **620 tests en 15 ficheros** (recuento reconciliado tras pytest global de Jay). *(Nota: `k8s/deployment-agent.yaml` sigue apuntando a `da7aafb` — actualizar tag en el siguiente commit.)*
...
- **En curso / siguiente**: (1) **S3·b — run `cured`**: `chaos-oom.yaml` sin `--timeout` (fix del falso-rollback, hecho) → run limpio a mano → aprobar 512Mi <60min TTL → veredicto `cured` → habilita R4; (2) R4 (métrica feedback-loop gain); (3) F-11/F-17 de la review; (4) F1 matriz E1–E6 (`docs/14`); Gate 8 resto de screenshots.
