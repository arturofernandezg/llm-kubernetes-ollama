...
- Sistema **completo y desplegado**; en cluster corre **`aiops-agent:2ac3c5d`** (S7, 2026-07-13). Fases 0-3 + Mini-Fase 4 + RAG + F2 (cola) + F3 (CPU + re-sourcing) + **v2 completa** (Ejes A+B) + **F4 R1/R2/R2·3/R3** (retrieval outcome-aware, p@1 73%→100%) + **F-03** (chroma no bloqueante) + **gap R2-humano cerrado** (`ca159be`) + **R4 corrido** (2026-07-09: Capa A retrieval 0→46.7% p@1; Capa B null pre-registrado — el 1.5b ignora la etiqueta de outcome) + **R5 bucle observacional** (validado 2026-07-13: `aiops_incident_resolution_seconds{OOMKilled}`=92.47s — esa serie ya no existe, TSDB stateless se evaporó con el churn del 07-14 F-27; **el flujo humano NO regenera R5**: el approve hace `pop` del índice → `resolved` da `miss` → en flujo humano la evidencia fuerte del Gate 8 es `feedback_verdict{cured}`, no R5) + **F-17** (logs+events al prompt) + **C-07** (pre-flight `auth can-i` free-text) + **C-08** (doble botón model/×2, HMAC por acción, **validado E2E 2026-07-13**) + **audit trail** (`7e40837`: log `AUDIT human decision` en approve/reject) + **fix C-08** (`2ac3c5d`: `sanitize_action_id` — MM valida `action_id` alfanumérico, el `_` daba 404 en MM). **Tests: 695 funciones en 17 ficheros; pytest colecta 700, verde Jay-confirmado 2026-07-15 (700 passed).** **Deck v3 "Observz"** (`demo/demo_v3.html` + `guion_v3.html`, 15+4 slides offline con replay embebido del run real + 23 QA tras la review hostil del 07-13) listo. Capturas S7: `mattermost_c08_double_button.png` + `mattermost_approved_patch_ok.png`.
...
- **S8 ✅ (2026-07-14/15)**: incidente nocturno + estabilización + arco `cured` limpio, **sin cambio de imagen** (`2ac3c5d`, solo manifiesto). Forense (F-19..F-27): **el agente llevaba una semana auto-remediándose a sí mismo** (mem 384Mi→10Gi, `fieldManager=kubectl-set`) — churn GKE → cold start colgado (F-20) → liveness-kill (F-19) → CrashLoop → auto-misdiagnóstico como memoria (el gate auto no exige `last_state_reason=OOMKilled`, F-26) → self-patch (F-21) → repeat. TSDB de Prometheus volátil (F-27: se evaporó con el churn, la serie R5 92.47s del 07-13 murió). Fix apply-only: `startupProbe /healthz` (300s, F-19) + limit reconciliado 2Gi (F-22); código F-20/F-21/F-26 escrito y testeado pero **NO desplegado** (no se estrena imagen la víspera). **Arco `cured` limpio 07-15 sobre `2ac3c5d`**: `approve_model` (512Mi) → patch → +300s health OK → `outcome=cured` + re-upsert ChromaDB; evidencia `feedback_verdict{cured}=1.0`, MTTD OOM 5.18s/CrashLoop 5.04s, MTTR OOM 264.4s/CrashLoop 133.4s, **700 passed**. Corrección de overclaim: **el flujo humano NO regenera R5** (approve→`pop`→`resolved`=miss; evidencia fuerte = `feedback_verdict{cured}`). Gotchas: runs solapados=colisión (un arco a la vez, no Ctrl-C a mitad de ventana), el `trap EXIT` no limpia `incident:active:*` ni `aiops:seen:*`, alertas `firing` sobreviven al pod ~15min.
- **En curso / siguiente**: chapter presentado el 15-jul (deck v3, arco cured limpio como evidencia). **Repo en limpieza pre-handoff a otro equipo (2026-07-15)**: deck v1 (`demo.html`/`guion.html`) + `build_demo.py` eliminados; README actualizado (módulos + 700 tests); docs consolidados vía `/promote`. Post-chapter: decidir orden v2.1 (F-15 → structured outputs → F-11 + AOF+PVC Redis → panel KPIs) + apuestas **ITBench** (puntuar el sistema en benchmark público) / **hardening adversarial** (threat-model de inyección vía logs/events al prompt, F-17); código pendiente F-20/F-21/F-26 (self-target + startup acotado + gate `OOMKilled`) con deploy post-chapter. Posicionamiento vs K8sGPT/HolmesGPT (diagnóstico) — nuestro diferencial es el actuador seguro con rollback verificado + aprendizaje del veredicto.
